...
async function weaningFindAnimalSrv(uid, rawNumber) {
  ...
  function packFound(id, data, collectionName) {
    return {
      id,
  ...
  if (typeof fetchAnimalByNumberForCalvingGateSrv === "function") {
    ...
    if (found) {
      ...
      return packFound(found.id, found.data || {}, collectionName);
    }
  }

  // 2) مهم للفطام: بعض العجول محفوظة برقمها كـ document id
  for (const colName of ["calves", "animals"]) {
    try {
      const directSnap = await db.collection(colName).doc(num).get();

      if (directSnap.exists) {
        const data = directSnap.data() || {};
        if (ownedByUser(data)) {
          return packFound(directSnap.id, data, colName);
        }
      }
    } catch (_) {}
  }

  // 3) fallback أقوى لحقول العجول القديمة/المستوردة
  const values = [num];
  const nNum = Number(num);
  if (Number.isFinite(nNum)) values.push(nNum);

  const collections = ["calves", "animals"];
  const fields = [
    "calfNumber",
    "animalNumber",
    "number",
      ...
      for (const field of fields) {
        for (const value of values) {
          try {
            ...
            if (!snap.empty) {
              const d = snap.docs[0];
              return packFound(d.id, d.data() || {}, colName);
            }
          } catch (_) {}
        }
      }
