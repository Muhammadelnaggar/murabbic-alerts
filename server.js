...
// ============================================================
//        API: SUPERNUMERARY TEAT REMOVAL GATE / SAVE
//        تسجيل إزالة الحلمات الزائدة من السيرفر فقط
// ============================================================

function superTeatStrSrv(v) {
  return String(v ?? "").trim();
}
...
function superTeatNumbersFromBodySrv(body = {}) {
  return superTeatParseNumbersSrv(
    body.numbers ||
    body.animalNumbers ||
    body.calfNumbers ||
    body.animalNumber ||
...
function superTeatEventDateSrv(body = {}) {
...
function superTeatIsFemaleSrv(doc = {}) {
  const raw = superTeatSexTextSrv(doc).toLowerCase();
  const n = raw
    .replace(/\s+/g, "")
    .replace(/[أإآ]/g, "ا")
    .replace(/ة/g, "ه")
...
function superTeatIsOutSrv(doc = {}) {
  const txt = [
    doc.status,
    doc.lifeStatus,
    doc.animalStatus,
...
function superTeatAlreadyMarkedSrv(doc = {}) {
  return !!String(
    doc.supernumeraryTeatRemovalDate ||
    doc.extraTeatRemovalDate ||
    doc.extraTeatsRemovedAt ||
    ""
  ).trim() || doc.supernumeraryTeatRemoved === true || doc.extraTeatsRemoved === true;
}

function superTeatIsEventSrv(ev = {}) {
  const txt = String(
    ev.eventTypeNorm ||
    ev.type ||
    ev.eventType ||
  ...
  return (
    txt === "supernumerary_teat_removal" ||
    txt.includes("supernumerary_teat") ||
    txt.includes("extra_teat") ||
    txt.includes("إزالة الحلمات الزائدة") ||
    txt.includes("حلمات زائدة")
  );
}

function superTeatEventIdSrv(uid, animalNumber) {
  const num = calvingNormDigitsOnlySrv(animalNumber);
  return ["supernumerary_teat_removal", uid, num].join("__");
}

async function superTeatFindCalfSrv(uid, rawNumber) {
...
async function superTeatHasPreviousEventSrv(uid, animalNumber) {
  const num = calvingNormDigitsOnlySrv(animalNumber);
  if (!db || !uid || !num) return false;

  try {
    const fixedSnap = await db.collection("events")
      .doc(superTeatEventIdSrv(uid, num))
      .get();
  ...
  for (const value of values) {
    try {
      ...
      if (!snap.empty && snap.docs.some(d => superTeatIsEventSrv(d.data() || {}))) {
        return true;
      }
    } catch (_) {}
  }
...
async function superTeatEvaluateOneSrv(uid, rawNumber, eventDate) {
  const num = calvingNormDigitsOnlySrv(rawNumber);
  ...
  const calf = await superTeatFindCalfSrv(uid, num);
  ...
  if (!superTeatIsFemaleSrv(doc)) {
    ...
    return {
      ...
      animalNumber: num,
      animalId: calf.id,
      reason: `إزالة الحلمات الزائدة تخص الإناث فقط — الجنس الحالي: ${sex}.`
    };
  }

  if (superTeatAlreadyMarkedSrv(doc)) {
    return {
      ok: false,
      animalNumber: num,
      animalId: calf.id,
      reason: "تم تسجيل إزالة الحلمات الزائدة لهذا العجل من قبل."
    };
  }

  if (await superTeatHasPreviousEventSrv(uid, num)) {
    return {
      ok: false,
      animalNumber: num,
      animalId: calf.id,
...
async function superTeatEvaluateManySrv(uid, numbers = [], eventDate = "") {
  ...
  for (const n of numbers) {
    const r = await superTeatEvaluateOneSrv(uid, n, eventDate);
  ...
  try {
    ...
    if (!calvingIsDateSrv(eventDate)) {
      return res.status(400).json({
        ...
        canSave: false,
        error: "invalid_date",
        message: "❌ تاريخ إزالة الحلمات الزائدة غير صالح.",
        accepted: [],
        rejected: []
      });
    }
    ...
    if (eventDate > superTeatTodaySrv()) {
      return res.status(400).json({
        ...
        canSave: false,
        error: "future_date",
        message: "❌ تاريخ إزالة الحلمات الزائدة لا يمكن أن يكون في المستقبل.",
        accepted: [],
        rejected: []
      });
    }
    ...
    console.error("supernumerary-teat-removal gate failed", e);
...
app.post("/api/supernumerary-teat-removal/save", requireUserId, async (req, res) => {
  try {
    if (!db) {
      return res.status(503).json({
        ok: false,
...
const LAMENESS_TYPES_SRV = [
  "مرض الخط الأبيض",
  "قرحة النعل",
  "كدمة أو نزيف في النعل",
  "خراج الحافر",
  "التهاب جلد الكعب / مورتيلاّرو",
  "تعفن القدم / التهاب بين الأظلاف",
  "تآكل الكعب",
  "تشقق جدار الحافر",
  "جرح نافذ أو جسم غريب",
  "التهاب الصفائح / خلل تكوين القرن",
  "عرج غير محدد يحتاج فحص الحافر"
];
...
const MASTITIS_TYPES_SRV = [
  "تحت ظاهري — الضرع طبيعي، اللبن طبيعي ظاهريًا، لا توجد جلطات لبنية، ويُكتشف بتحليل الخلايا أو اختبار الكأس",
  "خفيف — الضرع طبيعي أو به تورم بسيط، أول الحلب رقيق قليلًا، مع جلطات دبوسية دقيقة أو نقاط صغيرة، والحيوان طبيعي",
  "متوسط — ربع ساخن أو متورم أو مؤلم، اللبن سميك أو غير متجانس أو مائي مختلط، مع جلطات متوسطة أو تجبنات تشبه حبات الأرز",
  "شديد / كوليفورمي — ضرع شديد السخونة والتورم، اللبن مائي سيرمي شفاف أو أصفر قشّي، مع جلطات قليلة جدًا أو جلطات دبوسية دقيقة في سائل كهرماني",
  "مزمن / ليفي — ربع متليف أو منكمش أو متكتل، إفراز متقطع مائي أو صديدي، مع جلطات كبيرة متكتلة أو سدادات سميكة أو جلطات خيطية كريمية",
  "غرغريني — ربع بارد أزرق أو بنفسجي، إفراز مائي مدمم كريه الرائحة، مع أنسجة نخرية أو جلطات دموية داكنة"
];
...
function fecesRoundOneSrv(v) {
  const n = Number(v);
  if (!Number.isFinite(n)) return null;
  return Math.round(n * 10) / 10;
}

function fecesSessionLabelSrv(score) {
  const n = Number(score);
  if (!Number.isFinite(n)) return "غير محدد";
  return fecesLabelSrv(Math.max(1, Math.min(5, Math.round(n))));
}

function fecesNormalizeSamplesSrv(body = {}, fallbackAnalysis = {}, fallbackScore = null) {
  const rawSamples = Array.isArray(body.samples)
    ? body.samples
    : Array.isArray(body.sampleResults)
      ? body.sampleResults
      : Array.isArray(body.analyses)
        ? body.analyses
        : [];

  const source = rawSamples.length
    ? rawSamples
    : [{
        score: fallbackScore,
  ...
  return source
    .map((sample, idx) => {
      ...
      const analysis = item.analysis || item.result || item;
      ...
      return cleanObj({
        sampleNo: Number(item.sampleNo || item.no || idx + 1),
        score,
        fecesScore: score,
        label: item.label || analysis.label || fecesLabelSrv(score),
        quality: item.quality || analysis.quality || null,
        reason: item.reason || analysis.reason || null,
        findings: item.findings || analysis.findings || null,
        note: item.note || analysis.note || fecesBuildNoteSrv(score),
        rangeText: item.rangeText || analysis.rangeText || "1–5",
        source: item.source || analysis.source || "server:/api/feces/vision-analyze"
      });
    })
    .filter(Boolean);
}
...
app.post("/api/feces/save", requireUserId, async (req, res) => {
  try {
    ...
    const samples = fecesNormalizeSamplesSrv(body, analysis, fallbackScore);
    ...
    if (hasIndividualScope && samples.length !== 1) {
      return res.status(400).json({
        ok: false,
        message: "❌ التقييم الفردي يقبل عينة واحدة فقط."
      });
    }
...
if (hasGroupScope) {
  ...
  return res.status(groupResult.statusCode || 200).json(groupResult);
    ...
    const finalScore = hasGroupScope ? avgScore : scores[0];
    ...
    const payload = cleanObj({
      ...
      // الفردي هنا مرجع للعينة فقط — لا تحديث لوثيقة الحيوان
      linkedToAnimal: hasIndividualScope,
      animalId: hasIndividualScope ? (animalIdFromPage || null) : null,
      animalNumber: hasIndividualScope ? (animalNumber || null) : null,
      sampleAnimalNumber: hasIndividualScope ? (animalNumber || null) : null,

      groupId: hasGroupScope ? (groupId || null) : null,
      groupName: hasGroupScope ? (groupName || null) : null,
      feedingGroupId: hasGroupScope ? (groupId || null) : null,
      ...
      analysis: sampleCount === 1 ? samples[0] : null,
      sessionSummary: {
        sampleCount,
        avgScore,
        minScore,
