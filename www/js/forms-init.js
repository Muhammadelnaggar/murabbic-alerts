...
form.dataset.mbkOvsynchAttached = "1";
  ...
  if (!eventName) return;
// ======================================================
// Murabbik — Calving server-only path
// الواجهة هنا لا تتحقق ولا تقرر: تجمع البيانات وترسل للسيرفر فقط
// ======================================================
if (eventName === "ولادة") {
  form.dataset.locked = "0";

  // تأكد أن كل الحقول مفتوحة؛ لا Gate ولا قفل من الواجهة
  form.querySelectorAll("input, select, textarea, button").forEach((el) => {
    el.disabled = false;
  });
  ...
  form.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      showMsg(bar, data.message || "تم تسجيل الولادة بنجاح.", "success");
